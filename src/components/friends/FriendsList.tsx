...
export function FriendsList({ onSendMessage }: FriendsListProps) {
  ...
  const handleToggleBestFriend = async (friendshipId: string, current: boolean) => {
    ...
    } catch { toast({ title: "Kunde inte ändra bästis", variant: "destructive" }); }
  ...
  const handleRemove = async (friendshipId: string) => {
    ...
    } catch { toast({ title: "Kunde inte ta bort vän", variant: "destructive" }); }
    finally { setActionLoading(null); }
  };
  ...
  const handleCategoryChange = async (friendshipId: string, newCategory: string) => {
    try {
      ...
      if (error) throw error;
      setFriends(prev => prev.map(f => f.friendshipId === friendshipId ? { ...f, category: newCategory as FriendCategory } : f));
    } catch { toast({ title: "Kunde inte ändra kategori", variant: "destructive" }); }
  };
  ...
  if (isLoggedOut) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={() => navigate("/auth")} className="px-4 py-2 bg-primary text-primary-foreground font-bold text-sm uppercase">
          Logga in
        </button>
      </div>
    );
    ...
    <div className="w-full" style={{ fontFamily: "'Trebuchet MS', Arial, sans-serif" }}>
              ...
              >
                <div style={{ position: "relative", display: "inline-block" }}>
                  ...
                  <div style={{
                    ...
                    height: "10px",
                    borderRadius: "50%",
                    background: friend.status === "online" ? "#22c55e" : friend.status === "away" ? "#f59e0b" : "#6b7280",
                ...
                <div style={{ fontSize: "10px", color: "var(--primary)" }}>
                  <span style={{ cursor: "pointer" }} onClick={(e) => { e.stopPropagation(); navigate(`/profile/${encodeURIComponent(friend.username)}`); }}>Profil</span>
                  {" · "}
                  <span style={{ cursor: "pointer" }} onClick={(e) => { e.stopPropagation(); navigate(`/profile/${encodeURIComponent(friend.username)}?tab=gastbok`); }}>Gästbok</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{
        ...
        {(["all", "online", "best"] as const).map(f => (
          ...
          >
            {f === "all" ? `Alla (${acceptedFriends.length})` : f === "online" ? `Online (${onlineCount})` : "⭐ Bästisar"}
          </button>
        ))}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                  <thead>
                    <tr style={{ background: "var(--muted)", borderBottom: "1px solid var(--border)" }}>
                      <th style={{ padding: "3px 8px", textAlign: "left", fontWeight: "bold", fontSize: "10px", color: "var(--muted-foreground)", width: "32px" }}></th>
                      <th style={{ padding: "3px 8px", textAlign: "left", fontWeight: "bold", fontSize: "10px", color: "var(--muted-foreground)" }}>Användarnamn</th>
                      <th style={{ padding: "3px 8px", textAlign: "center", fontWeight: "bold", fontSize: "10px", color: "var(--muted-foreground)", width: "60px" }}>Online</th>
                      <th style={{ padding: "3px 8px", textAlign: "left", fontWeight: "bold", fontSize: "10px", color: "var(--muted-foreground)", width: "110px" }} className="hidden md:table-cell">Senast inloggad</th>
                      <th style={{ padding: "3px 8px", textAlign: "center", fontWeight: "bold", fontSize: "10px", color: "var(--muted-foreground)", width: "90px" }}>Åtgärd</th>
                    </tr>
                  </thead>
                  <tbody>
                    {catFriends.map((friend, idx) => (
                      ...
                      >
                        ...
                        <td style={{ padding: "4px 6px" }}>
                          <div style={{ position: "relative", display: "inline-block" }}>
                            <img
                              src={friend.avatar || `https://api.dicebear.com/7.x/initials/svg?seed=${friend.name}`}
                              alt={friend.name}
                              style={{ width: "24px", height: "24px", objectFit: "cover", border: "1px solid var(--border)", display: "block" }}
                              onClick={(e) => { e.stopPropagation(); navigate(`/profile/${encodeURIComponent(friend.username)}`); }}
                            />
                            <div style={{
                              position: "absolute",
                              bottom: "0",
                        ...
                        <td style={{ padding: "4px 8px" }}>
                          ...
                          {friend.isBestFriend && <Star size={10} style={{ display: "inline", marginLeft: "4px", color: "#eab308", fill: "#eab308" }} />}
                          {friend.statusMessage && (
                            <div style={{ fontSize: "10px", color: "var(--muted-foreground)", fontStyle: "italic", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "160px" }}>
                              {friend.statusMessage}
                            </div>
                          )}
                        </td>
                        ...
                        <td style={{ padding: "4px 8px", textAlign: "center" }}>
                          <div style={{
                            ...
                            background: friend.status === "online" ? "#22c55e" : friend.status === "away" ? "#f59e0b" : "#6b7280",
                            margin: "0 auto"
                          }} title={friend.status === "online" ? "Online" : friend.status === "away" ? "Borta" : "Offline"} />
                        </td>

                        {/* Last seen - hidden on mobile */}
                        <td style={{ padding: "4px 8px", fontSize: "11px", color: "var(--muted-foreground)" }} className="hidden md:table-cell">
                        ...
                        <td style={{ padding: "4px 6px", textAlign: "center" }} onClick={e => e.stopPropagation()}>
                          <div style={{ display: "flex", gap: "4px", justifyContent: "center" }}>
                            ...
                            <button
                              ...
                              style={{ background: "none", border: "none", cursor: "pointer", color: friend.isBestFriend ? "#eab308" : "var(--muted-foreground)", padding: "2px" }}
                            >
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", gap: "16px", alignItems: "flex-start", flexWrap: "wrap" }}>
                      <div>
                        ...
                        >
                          <SelectTrigger style={{ height: "24px", width: "130px", fontSize: "11px", display: "inline-flex" }}>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            {FRIEND_CATEGORIES.map(cat => (
                              <SelectItem key={cat} value={cat} style={{ fontSize: "11px" }}>{cat}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </div>
                    ...
                    <tr key={friend.id} style={{ background: "var(--card)", borderBottom: "1px solid var(--border)" }}>
                      ...
                      <td style={{ padding: "6px 8px" }}>
                        <div style={{ display: "flex", gap: "6px" }}>
                          <button
                            onClick={async () => {
                              await supabase.from("friends").update({ status: "accepted" }).eq("id", friend.friendshipId);
                              toast({ title: "Vänförfrågan accepterad!" });
                            }}
                            style={{ background: "#22c55e", color: "#fff", border: "none", padding: "2px 8px", fontSize: "11px", fontWeight: "bold", cursor: "pointer" }}
                          >
                            ✓ Acceptera
                          </button>
                          <button
                            ...
                            style={{ background: "#ef4444", color: "#fff", border: "none", padding: "2px 8px", fontSize: "11px", fontWeight: "bold", cursor: "pointer" }}
                          >
                            ✕ Avböj
                          </button>
                        </div>
