...
export function Header({
  ...
  const privateZoneItems: {
    ...
    label: "GÄST",
    emoji: "👣",
    animationClass: "footsteps"
  }, {
    id: "mejl",
    label: "MEJL",
    emoji: "✉️",
    animationClass: "msn-bounce"
  }, {
    id: "chatt",
    label: "EMN",
    emoji: "🖊️",
    animationClass: "writing-pen"
  }, {
    id: "vanner",
    label: "VÄNNER",
    emoji: "❤️",
    animationClass: "heart-pulse"
  }, {
    id: "profil",
    label: "PROFIL",
    emoji: "👤",
  ...
  const renderNavItem = (item: {
    ...
    return <div key={item.id} onClick={() => onTabChange?.(item.id)} className={cn(isHome ? "nav-item-home" : "nav-item-grouped", "shrink-0", activeTab === item.id && "active", !isHome && (hasNotice ? "has-notice" : "inactive"))} role="button" tabIndex={0} aria-label={item.label}>
  ...
  return <header className="sticky top-0 z-50">
      ...
      <div className="navbar-dark">
        ...
        <div className="flex items-center gap-1 sm:gap-2 lg:gap-4 ml-auto">
          {!loading && <>
              {user ? <>
                  {isAdmin && <Button variant="ghost" size="sm" onClick={() => navigate("/admin")} className="text-foreground hover:bg-muted text-xs gap-1 px-2 sm:px-3">
                      <Shield className="w-4 h-4" />
                      <span className="hidden sm:inline">Admin</span>
                    </Button>}
                  <Button variant="ghost" size="sm" onClick={handleSignOut} className="text-foreground hover:bg-muted text-xs gap-1 px-2 sm:px-3">
                    <LogOut className="w-4 h-4" />
                    <span className="hidden sm:inline">Logga ut</span>
                  </Button>
                </> : <Button variant="ghost" size="sm" onClick={() => navigate("/auth")} className="text-foreground hover:bg-muted text-xs gap-1 px-2 sm:px-3">
