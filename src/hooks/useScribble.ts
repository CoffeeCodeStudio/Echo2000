 ...
 * Adapted for Echo2000: uses userId and username params instead of useGuestId.
 */
export function useScribbleGame(lobbyId: string | null, userId: string, username: string) {
  ...
  const displayName = username || 'Gäst';
  ...
  const submitGuess = async (guess: string) => {
    ...
    if (isCorrect) {
      await supabase.from('scribble_players')
        .update({ score: (players.find(p => p.user_id === userId)?.score || 0) + 10 })
        .eq('lobby_id', lobbyId)
        .eq('user_id', userId);
    }
  ...
  const leaveLobby = async () => {
    ...
    await supabase.from('scribble_guesses').insert({
      ...
      user_id: userId,
      username: displayName,
      guess: `📤 ${displayName} lämnade spelet`,
      is_correct: false,
    });

    // Remove the player
    await supabase.from('scribble_players').delete().eq('lobby_id', lobbyId).eq('user_id', userId);
    ...
    if (remainingPlayers.length === 0) {
      ...
      if (lobby?.current_drawer_id === userId && lobby?.status === 'playing') {
        updates.current_drawer_id = remainingPlayers[0].user_id;
        updates.current_word = null;
  ...
  return { lobby, players, guesses, joinLobby, submitGuess, leaveLobby, visitorId: userId };
}
