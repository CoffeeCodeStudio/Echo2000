...
CREATE OR REPLACE FUNCTION public.clear_all_bot_activity(p_category text DEFAULT 'all'::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_news_comments_count integer := 0;
  v_klotter_count integer := 0;
...
BEGIN
  ...
  IF p_category IN ('all', 'news_comments') THEN
    WITH deleted AS (
      DELETE FROM news_comments
      WHERE user_id IN (SELECT user_id FROM profiles WHERE is_bot = true)
      RETURNING id
    ) SELECT count(*) INTO v_news_comments_count FROM deleted;
  END IF;

  IF p_category IN ('all', 'klotter') THEN
    WITH deleted AS (
      DELETE FROM klotter
      WHERE user_id IN (SELECT user_id FROM profiles WHERE is_bot = true)
      RETURNING id
    ) SELECT count(*) INTO v_klotter_count FROM deleted;
  END IF;

  IF p_category IN ('all', 'friends') THEN
    WITH deleted AS (
      DELETE FROM friends
      WHERE user_id IN (SELECT user_id FROM profiles WHERE is_bot = true)
         OR friend_id IN (SELECT user_id FROM profiles WHERE is_bot = true)
      RETURNING id
    ) SELECT count(*) INTO v_friends_count FROM deleted;
  END IF;

  IF p_category IN ('all', 'profile_visits') THEN
    WITH deleted AS (
      DELETE FROM profile_visits
      WHERE visitor_id IN (SELECT user_id FROM profiles WHERE is_bot = true)
      RETURNING id
    ) SELECT count(*) INTO v_visits_count FROM deleted;
  END IF;
  ...
  RETURN json_build_object(
    ...
    'deleted', json_build_object(
      ...
      'news_comments', v_news_comments_count,
      'klotter', v_klotter_count,
      'friends', v_friends_count,
      'profile_visits', v_visits_count
    )
  );
END;
$function$;
